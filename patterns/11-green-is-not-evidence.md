...
A presence test — *the response contains the resolved display name* — is coupled to what it checks:
break the code that produces the name and the assertion has nothing to match.

An absence test — *the credential does not appear in the log* — passes by finding nothing, and finds
nothing in two worlds: where the guard works, and where the code that would have produced the
forbidden thing never ran. Both emit the same green tick.

## Forces

**Absence is how you state the properties you care most about.** Security and privacy requirements
are negative — must not leak, must not accept, must not persist. The requirements with the worst
consequences produce the test class with the weakest evidence.

**Every other test class earns trust honestly,** so the habit of reading green as evidence transfers
intact into the one place it doesn't hold. Nobody investigates a passing test.

**Refactors remove code paths quietly** — a call inlined, flagged off, or dropped in a diff about
something else. That surfaces not as a failing absence test but as an absence test that got easier to
pass.
...
2. **Fail for the right reason.** A test that errors because the fixture stopped building has told
   you nothing about the property.
3. **Record it.** Unrecorded verification decays to folklore.

**"The test passed" is a fact about this run. "The test would have failed" is a fact about the
test.** Only the second is evidence, and only it survives the refactor that hasn't happened yet.

### The worked example

A service logs request metadata; a test drives one request, captures the log stream, and asserts the
API key does not appear in it. Green from day one.

Months later a refactor drops the call site — one line, in a diff about something else. The handler
now logs nothing, so the assertion is trivially true: what the suite reports as a covered property is
that there was no output to search. Reintroduce logging without the redaction and the test stays
green, with nothing marking the moment it stopped meaning anything.
...
Put the absence in the type instead: a credential wrapper whose unwrapping is explicit and audited,
and a logging interface accepting only values that render — which the wrapper does not. **A test says
the leak did not occur in the cases you ran; a type says the code expressing the leak does not
compile.** Where types cannot reach, the mutation check is the fallback, not the ideal.

### Past the test suite

Any all-clear produced by the same code path as *not configured* has this defect: a scanner whose
credential expired and now returns zero findings. The healthy signal and the broken signal are the same signal, and the broken
one is quieter. **An alert you have never seen fire is an alert you have not tested.**
...
record. Mutation frameworks automate the wrong part: the mutants that matter are semantic, and come
from someone who knows what the guard is for.
...
**Coverage records that a line executed, not that any assertion depended on it.** In the worked
example, coverage of the handler stays high throughout: the line that vanished was not an assertion,
and no coverage number is computed over assertions. Coverage answers *did this code run*; the
question was *would this test have noticed*.
...
- DeMillo, Lipton & Sayward, *Hints on Test Data Selection: Help for the Practicing Programmer* —
  IEEE *Computer*, April 1978. The origin of mutation testing: a test that no fault can break is not
  a test
...
state poisoned, so it compared the poisoned value to itself and passed. It now asserts against a
private sentinel.
